...
const PostJob = () => {
  ...
  const addPerk = (perk) => {
    ...
    if (!form.perks.includes(perk)) {
      setForm((prev) => ({ ...prev, perks: [...prev.perks, perk] }));
    }
    setPerkInput("");
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const payload = {
        ...form,
        minExp: form.minExp ? Number(form.minExp) : 0,
        maxExp: form.maxExp ? Number(form.maxExp) : 0,
        minSalary: form.minSalary ? Number(form.minSalary) : 0,
        maxSalary: form.maxSalary ? Number(form.maxSalary) : 0,
        skills: form.skills || [],
        perks: form.perks || [],
        responsibilities: form.responsibilities || [],
        consultancyName: jobType === "consultancy" ? form.consultancyName : "",
      };
      ...
      if (!res.ok) {
        ...
        throw new Error(errMsg.message || "Failed to post job");
      }
    ...
    <>
      ...
      <div className="flex items-center justify-center min-h-screen px-4 text-white">
        ...
        >
          ...
          {/* Work Mode, Department, Location, etc */}
          <input name="workMode" placeholder="Work mode" className={inputStyle} onChange={handleChange} />
          ...
          <input name="companyType" placeholder="Company type" className={inputStyle} onChange={handleChange} />
          ...
          {/* Skills */}
          <div className="mb-6">
            <p className="mb-2 font-semibold text-slate-300">Required Skills</p>
            <input
              value={skillInput}
            ...
            <div className="flex flex-wrap gap-2">
              {form.skills.map((skill) => (
                <span key={skill} className="flex items-center gap-2 px-3 py-1 text-sm text-black rounded-full bg-cyan-500">
          ...
          {/* Salary */}
          <div className="grid grid-cols-2 gap-4 mb-4">
            <input type="number" name="minSalary" placeholder="Min Salary (LPA)" className={inputStyle} min="0" step="0.1" onChange={handleChange} />
            <input type="number" name="maxSalary" placeholder="Max Salary (LPA)" className={inputStyle} min="0" step="0.1" onChange={handleChange} />
          </div>

          {/* Perks */}
          <div className="mb-6">
            <p className="mb-2 font-semibold text-slate-300">Perks & Benefits</p>
            <select className={inputStyle} onChange={(e) => addPerk(e.target.value)} defaultValue="">
              <option value="" disabled>Select a benefit</option>
              {perkSuggestions.map((perk) => <option key={perk} value={perk}>{perk}</option>)}
            </select>
            <input
              value={perkInput}
              ...
              className={inputStyle}
              onKeyDown={(e) => {
                if (e.key === "Enter" && perkInput) { e.preventDefault(); addPerk(perkInput); }
              }}
            />
            <div className="flex flex-wrap gap-2">
              {form.perks.map((perk) => (
                <span key={perk} className="flex items-center gap-2 px-3 py-1 text-sm text-black bg-teal-500 rounded-full">
                  {perk} <button onClick={() => removePerk(perk)}>✕</button>
                </span>
              ))}
            </div>
          </div>
          ...
          {/* Job Description */}
          <textarea name="jobDescription" placeholder="Job description" rows="4" className={inputStyle} onChange={handleChange} />

          {/* Responsibilities */}
          <textarea placeholder="Responsibilities (one per line)" rows="4" className={inputStyle}
            onChange={(e) => setForm((prev) => ({ ...prev, responsibilities: e.target.value.split("\n") }))}
          />

          {/* Apply Before */}
          <input type="date" name="applyBefore" className={inputStyle} onChange={handleChange} />
